...
public class GameWorld
{
	...
	private void resolveManageTeam(String input)
	{
		...
		try
		{
			...
			trainer.useItem(choice[1],monster);
		}
		catch (NumberFormatException e)
		{
			if (monster != UNSELECTED)
			{
				System.out.print(trainer.getMonsters().get(monster).getName() + " is now called ");
				trainer.getMonsters().get(monster).setName(choice[1]);
				System.out.println(trainer.getMonsters().get(monster).getName());
			}
		}
		catch (Exception e)
		{
	...
	private void resolveStartup(String input)
	{
		try
		{
			...
			case "1":
				...
				break;
			case "2":
				trainer.addMonster(new Dragonfish(choice[1], trainer.getTrainerLevel(), trainer.getTrainerLevel() * 12));
				trainer.addItem("HEALING_POTION");
				break;
			case"3":
				trainer.addMonster(new Seedatops(choice[1], trainer.getTrainerLevel(), trainer.getTrainerLevel() * 12,2));
				trainer.addItem("HEALING_POTION");
				break;
			}
			...
			System.out.println("The first member of your team! " + NEWLINE
					...
					+ "You also got a Healing Potion to help you on your way!");
			gameMode = Mode.EXPLORE;
		}
		catch (Exception e)
		{
	...
	private void resolveAfterWild(String input)
	{
		try
		{
			...
			{
			case "1":
				trainer.getMonsters().add(wildMonster);
	...
	private void ouputStartupChoices()
	{
		...
		System.out.println("2 - A Dragonfish, a fierce aquatic monster with a very powerful water attack!");
	}
